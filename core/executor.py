...
Fluxo v0.4:
  busca → análise → priorização → abordabilidade → diagnóstico → abordagem → salvamento
...
def executar() -> None:
    ...
    logger.info("ETAPA 2/7 - Analisando presenca digital...")
    ...
    logger.info("ETAPA 3/7 - Calculando prioridade comercial...")
    ...
    logger.info("ETAPA 4/7 - Calculando abordabilidade...")
    ...
    logger.info("ETAPA 5/7 - Gerando diagnosticos...")
    empresas = diagnosticar_empresas(empresas)
    logger.info("ETAPA 5 concluida.")

    # ETAPA 6: Abordagem — gera pacote de mensagens para empresas abordáveis
    logger.info("ETAPA 6/7 - Gerando pacotes de abordagem...")

    # candidatas_abordaveis_pre é a lista antes de enriquecer com abordagem
    ...
    # ETAPA 7: Salvamento — 5 arquivos com logica explicita
    logger.info("ETAPA 7/7 - Salvando resultados...")
    ...
    # (usa a lista já gerada na etapa 6, sem pacote de abordagem — referência simples)
    ...
    logger.info(f"  candidatas_com_abordagem   : {len(candidatas_com_abordagem)} (com pacote de mensagens)")
